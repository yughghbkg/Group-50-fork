...
class LostDetector:
    def __init__(self, robot, time_step=64):
        ...
        # Track consecutive "no movement" detections
        self.stall_count = 0
        self.stall_threshold = 20  # need N consecutive stalls to drop confidence
    ...
    def check(self, localiser=None):
        ...
        # consecutive stall check: only when we see repeated "moving but static env" OR "not moving with IR spike"
        ...
        self.is_lost = (self.confidence < self.lost_threshold and self.stall_count >= self.stall_threshold)
